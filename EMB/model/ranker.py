...
class Conv2dRanker(nn.Module):
    ...
    def compute_loss(moments, bounds, scores, mask, min_iou=0.3, max_iou=0.7, noise_alpha=0.75, noise_beta=0.75, use_reweight=False):
        ...
        # loc_acc_r = ious * mask
        loc_acc_r = loc_acc_r / (torch.sum(loc_acc_r, dim=-1, keepdim=True) / torch.sum(mask, dim=-1, keepdim=True))
        cls_conf_r = 1 / (1 - scores.detach() + 1e-9) * mask
        # cls_conf_r = scores.detach() * mask
        cls_conf_r = cls_conf_r / (torch.sum(cls_conf_r, dim=-1, keepdim=True) / torch.sum(mask, dim=-1, keepdim=True))
        ...
        loss = F.binary_cross_entropy(scores, targets, reduction='none') * mask
        if use_reweight:
            tgt_idx = scores.argmax(dim=-1)
            weight = reweight.gather(dim=-1, index=tgt_idx.unsqueeze(-1))
            loss_scale = loss.sum().data / ((loss * weight).sum().data + 1e-9)
            loss = loss * weight * loss_scale
        else:
            weight = None

        soft_labels = []
        softscore = (noise_beta * ious.detach() + (1-noise_beta) * scores.detach()) * mask
        soft_idx = softscore.argmax(dim=-1)
        for i, idx in enumerate(soft_idx):
            soft_labels.append(bounds[i, idx].cpu().tolist())

        return torch.sum(loss) / torch.sum(mask), weight, soft_labels