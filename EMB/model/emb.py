...
class EMB(nn.Module):
    ...
    def compute_ranking_loss(self, moments, proposals, scores, mask, min_iou, max_iou, noise_alpha=0.75, noise_beta=0.75, use_reweight=False):
        return self.ranker.compute_loss(
            moments=moments, bounds=proposals, 
            scores=scores, mask=mask, min_iou=min_iou, max_iou=max_iou, use_reweight=use_reweight)

    def compute_highlight_loss(self, scores, labels, mask, moments, 
                    proposals, pscores, pmask, threshold=0.5, extend=0.1, sample_weight=None):
        ...
        return self.highlight_layer.compute_loss(scores=scores, labels=labels, mask=mask, sample_weight=sample_weight)

    def compute_loss(self, start_logits, end_logits, start_labels, end_labels,
            proposals, pscores, pmask, threshold=0.5, sample_weight=None):
        ...
        start_loss = (F.softmax(start_logits, dim=-1) * start).sum(-1).log()
        end_loss = (F.softmax(end_logits, dim=-1) * end).sum(-1).log()
        if sample_weight is not None:
            sample_weight = sample_weight.squeeze(-1)
            loss_scale = start_loss.sum().data / ((start_loss * sample_weight).sum().data + 1e-9)
            start_loss = start_loss * sample_weight * loss_scale
            loss_scale = end_loss.sum().data / ((end_loss * sample_weight).sum().data + 1e-9)
            end_loss = end_loss * sample_weight * loss_scale
