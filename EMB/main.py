...
def generate_h_labels(s_idx, e_idx, vfeat_len):
    extend = 0.1
    h_labels = torch.zeros(vfeat_len, dtype=torch.long)
        ...
        for local_step, data in enumerate(tqdm(train_loader, total=num_train_batches, 
            ...
            if epoch > configs.epochs // 2:
                for idx in range(len(records)):
                    if records[idx]['sample_id'] not in soft_labels_last:
                        continue
                    s_labels[idx] = soft_labels_last[records[idx]['sample_id']][0]
                    e_labels[idx] = soft_labels_last[records[idx]['sample_id']][1]
                    h_labels[idx][:vfeat_lens[idx]] = soft_labels_last[records[idx]['sample_id']][2]
            # prepare metd
            ...
            rank_loss, sample_weight, soft_labels = model.compute_ranking_loss(moments, p_time, p_score, 
                        p_mask, min_iou=configs.min_iou, max_iou=configs.max_iou, noise_alpha=configs.noise_alpha, noise_beta=configs.noise_beta, use_reweight=epoch > configs.epochs // 2)
            for idx in range(len(soft_labels)):
                s_ind, e_ind, _ = time_to_index(soft_labels[idx][0], soft_labels[idx][1], vfeat_lens[idx].item(), records[idx]['duration'])
                soft_labels_last[records[idx]['sample_id']] = [s_ind, e_ind, generate_h_labels(s_ind, e_ind, vfeat_lens[idx].item())]
            highlight_loss = model.compute_highlight_loss(h_score, h_labels, video_mask,
                ...
                threshold=threshold(epoch, reverse=True), extend=configs.extend, sample_weight=sample_weight)
            loc_loss = model.compute_loss(start_logits, end_logits, s_labels, 
                e_labels, proposals, p_score, p_mask,
                threshold=threshold(epoch, reverse=True), sample_weight=sample_weight)
            total_loss = (loc_loss + 
