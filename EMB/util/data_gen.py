...
class CharadesProcessor:
    ...
    def convert(self, data_dir):
        ...
        train_data = load_lines(os.path.join(data_dir, 'charades_sta_train_pseudo.txt'))
...
class ActivityNetProcessor:
    ...
    def convert(self, data_dir):
        ...
        train_data = load_json(os.path.join(data_dir, 'train_pseudo.json'))
        val_data = load_json(os.path.join(data_dir, 'val_1.json'))
        test_data = load_json(os.path.join(data_dir, 'val_2.json'))
        # process data
...
def gen_or_load_dataset(configs):
    ...
    if configs.suffix is None:
        save_path = os.path.join(configs.save_dir, '_'.join([configs.task, 
            configs.fv, str(configs.max_pos_len)]) + '.pkl')
    else:
        save_path = os.path.join(configs.save_dir, '_'.join([configs.task, 
            configs.fv, str(configs.max_pos_len), 
            configs.suffix]) + '.pkl')
